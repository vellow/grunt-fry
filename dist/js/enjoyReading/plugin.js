	...
	if(typeof (s) != "undefined") {
		...
		for(var i = 0; i < sArr.length; i++){
			if(uSinaEmotionsHt.containsKey(sArr[i])) {
				var reStr = "<img style='display:inline;' alt=\"AnalyticEmotionsArr["+i+"]\" src=\"" + uSinaEmotionsHt.get(sArr[i]) + "\" height=\"22\" width=\"22\" />";
				s = s.replace(sArr[i], reStr);
			}
		}
		for(var i = 0; i < sArr.length; i++){
			s = s.replace('AnalyticEmotionsArr['+i+']', sArr[i]);
		}
	}
	return s;
}
